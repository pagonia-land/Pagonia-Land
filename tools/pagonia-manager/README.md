...
- [`CHANGELOG.md`](CHANGELOG.md) tracks user-visible behaviour changes per release — see it for the per-release history (live game-install deploy and its optimisations, the remote-source + catalog work, the interactive-menu restructure, install-state integrity + version-awareness, transparent mod/collection updates, and dependency / incompatibility / load-order handling).
...
- **Active-profile mod selection** — `enable` / `disable` / `move --before/--after/--position` / `status`. The active profile's `loadOrder` directly drives apply order at deploy time, and the enabled set is **auto-ordered** by each mod's declared `loadAfter` / `loadBefore` (your manual order as the stable tiebreaker; a reorder is reported as `manager.loadOrderAdjusted`, a constraint cycle as `manager.loadOrderCycle`).
- **Dependencies + incompatibilities** — the manager reads each mod's `dependencies` / `incompatibleWith` and warns (advisory, never blocking) when a required mod isn't enabled (`manager.modDependencyMissing`) or two enabled mods clash (`manager.modIncompatibleEnabled`), and before you disable/uninstall a mod others still depend on (`manager.modDependedUponByOthers`). `install --with-deps` pulls a mod's missing dependencies (transitively) from the same repo, then your subscribed catalogs.
- **Updates** — `outdated` is a read-only "is anything I have out of date?" check across your `gh:`-sourced mods and collections (including same-version **content drift** via the index `contentHash`); `update <mod-id>` and `collection update <id>` are the transparent, **reversible** apply step (the new version installs alongside the old rollback anchor, your tweak overrides carried forward). All three are also driven from the interactive *Update mods + collections* screen, and `doctor --check-updates` folds the count into the health roll-up.
- **Health roll-up** — `doctor [--game <path>] [--check-updates]` is the scriptable one-stop check: store initialised, active profile, every enabled mod installed, dependencies + incompatibilities, cross-mod overlay conflicts, orphaned deploys, deploy-backup storage, and (with a game root) expansion ownership. Each is `[OK]`/`[WARN]`/`[FAIL]`/`[SKIP]` with a summary; exits non-zero only on an error, so it drops cleanly into CI or a pre-deploy script.
