...
| Pick | What it does |
|---|---|
| **Install a mod** | Asks for a folder, `.zip`, GitHub repo, mod.io coordinate, or direct-download URL; validates through the patcher pipeline; installs; offers to enable it in the active profile in one step |
...
| Pick | What it does |
|---|---|
| **Status dashboard** | Read-only overview: store + active profile + installed mods + last deploy. Surfaces warning panels when at least one orphaned deploy exists (game install moved or updated) or when `<store>/deploys/` exceeds ~5 GB |
